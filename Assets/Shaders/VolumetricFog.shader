    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) :   SV_Target
            {
                ...
                //space effect
                float spaceTime = 1- clamp(InverseLerp(_CutoffStart, _CutoffEnd, worldPos.y),0,1);
                spaceTime *= (end ? 0 : 1);
                //screenEffect
                float screenTime = 1- clamp(InverseLerp(_CutoffStart, _CutoffEnd, entryPoint.y),0,1);
                //float screenTime = 1 - clamp(entryPoint.y ,_CutoffStart,_CutoffEnd) / _CutoffEnd;    
                //preventing overlap of effects
                //screenTime *=  spaceTime;
                //return col * spaceTime;
               // return lerp(col, _Color, (1- saturate(transmittance)) * screenTime);
                return lerp(col, _Color, (1- saturate(transmittance)) * (spaceTime  + screenTime *_InsideStrength)*.5f * _Color.w);
                //return lerp(col, _Color, (1- saturate(transmittance)) );
                //float4 endCol = _Color * (screenTime + spaceTime) * 0.5f;
                //endCol.w = 1;
